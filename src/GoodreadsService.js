...
class GoodreadsService {
  constructor(jwt, apiBaseUrl) {
    ...
    if (!this.jwt) {
      localForage.getItem('jwt').then(localStorage_jwt => {
        if (localStorage_jwt) {
  ...
  cacheShelf(shelf) {
    ...
    localForage.setItem(
      this.shelfCacheName(shelf.name, shelf.pagination.currentpage),
      shelfData
    );
    return shelf;
  }
    ...
    for (var shelfElement of shelvesElements) {
      shelves.push(
        Object.assign({}, Shelf, {
          ...
          book_count: Number(
            shelfElement.getElementsByTagName('book_count')[0].textContent
          ),
          description: shelfElement.getElementsByTagName('description')[0]
            .textContent
          // sort:
          // order:
          // per_page:
  ...
  shelfFromGoodreadsXml(xmlStr) {
    ...
    for (var bookElement of booksElements) {
      ...
      books.push(
        Object.assign({}, Book, {
          ...
          title_without_series: bookElement.getElementsByTagName(
            'title_without_series'
          )[0].textContent,
          image_url: bookElement.getElementsByTagName('image_url')[0]
            .textContent,
          small_image_url: bookElement.getElementsByTagName(
            'small_image_url'
          )[0].textContent,
          ...
          published: bookElement.getElementsByTagName('published')[0]
            .textContent,
          ...
          description: bookElement.getElementsByTagName('description')[0]
            .textContent,
          authors
        })
      );
    ...
    let pagination = {
      ...
      currentpage: parseInt(
        booksContainerElement.getAttribute('currentpage'),
        10
      )
    };
