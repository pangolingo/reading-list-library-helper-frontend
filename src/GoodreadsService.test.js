import GoodreadsService, {
  GoodreadsException,
  GoodreadsUnauthenticatedException
} from './GoodreadsService';
import fs from 'fs';
...
describe('parseShelf', () => {
  it('returns an array of books', () => {
    let shelfXML = fs.readFileSync(
      __dirname + '/test/fixtures/shelf.xml',
      'utf8'
    );
  ...
  it('returns books with the right keys', () => {
    ...
    expect(firstBook.title_without_series).toBe(
      'Jonathan Strange & Mr Norrell'
    );
    expect(firstBook.image_url).toBe(
      'http://images.gr-assets.com/books/1357027589m/14201.jpg'
    );
    expect(firstBook.small_image_url).toBe(
      'http://images.gr-assets.com/books/1357027589s/14201.jpg'
    );
    ...
    expect(firstBook.description).toBe(
